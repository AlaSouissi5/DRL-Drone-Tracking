...
from .customCNN import Resnet_rgb, Resnet_one_channel , Resnet_two_channel
...
class SACDensePolicy(SACPolicy):
    ...
    def make_actor(self, features_extractor: Optional[BaseFeaturesExtractor] = None) -> DenseActor:
        ...
        # obs actor shape : (3, C, H, W)
        if observation_space['actor'].shape[1] == 3 :
          features_extractor = Resnet_rgb(observation_space['actor'])
        elif observation_space['actor'].shape[1] == 1 :
          features_extractor = Resnet_one_channel(observation_space['actor'])
        elif observation_space['actor'].shape[1] == 2 :
          features_extractor = Resnet_two_channel(observation_space['actor'])
...
register_policy("Resnet_DenseMlpPolicy", SACDensePolicy)